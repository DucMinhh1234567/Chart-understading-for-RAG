...
class BarChartExtractor:
    ...
    def _extract_ylabel_from_left_region(self, image: np.ndarray) -> Optional[str]:
        ...
        # Crop vùng bên trái
        left_region = image[
            int(DEFAULT_LAYOUT.LEFT_REGION_MIN_Y * h) : int(
                DEFAULT_LAYOUT.LEFT_REGION_MAX_Y * h
            ),
        ...
        results = self.ocr.read_text_easyocr(
            rotated, confidence_threshold=DEFAULT_LAYOUT.YLABEL_CONFIDENCE
        )
    ...
    def _extract_labels(
        ...
        if (
            not labels.get("ylabel")
            or labels.get("ylabel") == "None"
            or labels.get("ylabel") == ""
        ):
    ...
    def _build_output(
        ...
        for i, bar in enumerate(bars):
            ...
            structured_data["data"].append({"category": cat, "value": val})
    ...
    def extract(self, image_path: str, ocr_method: str = "easyocr") -> Dict[str, Any]:
        ...
        categories = self._extract_categories(
            labels, bars, image.shape[1], image.shape[0]
        )
    ...
    def _detect_y_axis_scale(
        self, image: np.ndarray, y_axis: Optional[np.ndarray], labels: Dict[str, Any]
        ...
        for val_info in labels.get("values", []):
            ...
            if x < DEFAULT_LAYOUT.YTICK_REGION_MAX_X * w and val_info.get(
                "is_number", False
            ):
                try:
                    tick_value = float(text.replace(",", "").replace("%", ""))
                    y_ticks.append({"value": tick_value, "y_position": y})
                except ValueError:
                    continue
    ...
    def _extract_bar_values_from_labels(
        self, bars: List[Dict[str, Any]], labels: Dict[str, Any], image: np.ndarray
    ) -> List[Optional[float]]:
        """
        ...
        value_labels = [
            v for v in labels.get("values", []) if v.get("is_number", False)
        ]
        ...
        for bar in bars:
            ...
            for label_info in value_labels:
                ...
                if (
                    x_dist < DEFAULT_LAYOUT.VALUE_X_THRESHOLD
                    and y_dist < DEFAULT_LAYOUT.VALUE_Y_THRESHOLD
                ):
            ...
            if closest_label and min_dist < DEFAULT_LAYOUT.VALUE_TOTAL_DIST_THRESHOLD:
                ...
                try:
                    value_text = (
                        closest_label["text"].replace(",", "").replace("%", "").strip()
                    )
    ...
    def _calculate_values_from_pixels(
        ...
        def pixel_to_value(pixel_y: int) -> float:
            ...
            if len(tick_positions) < 2:
                ...
                pixel_range = (
                    baseline_y - tick_positions[0]
                    if tick_positions
                    else h - DEFAULT_LAYOUT.FALLBACK_BASELINE_OFFSET
                )
            ...
            if pixel_y < tick_positions[0]:
                ...
                y1, y2 = (
                    tick_positions[0],
                    tick_positions[1] if len(tick_positions) > 1 else tick_positions[0],
                )
                v1, v2 = (
                    tick_values[0],
                    tick_values[1] if len(tick_values) > 1 else tick_values[0],
                )
                ratio = (pixel_y - y1) / (y2 - y1) if (y2 - y1) > 0 else 0
                return v1 + ratio * (v2 - v1)
            else:
    ...
    def _calculate_bar_values(
        ...
        values_from_pixels = self._calculate_values_from_pixels(
            bars, y_axis, image, labels
        )
    ...
    def _extract_categories(
        ...
        for text_info in all_texts:
            ...
            if (
                y > DEFAULT_LAYOUT.CATEGORY_REGION_MIN_Y * img_height
                and not text_info.get("is_number", False)
                and text != xlabel
                and len(text) > 0
            ):
        ...
        category_candidates = sorted(
            category_candidates, key=lambda t: t["position"][0]
        )
        ...
        for i, bar in enumerate(bars):
            ...
            for idx, text_info in enumerate(category_candidates):
                ...
                threshold = (
                    ...
                    if len(bars) > 0
                    else DEFAULT_LAYOUT.MIN_SPACING_THRESHOLD
                )

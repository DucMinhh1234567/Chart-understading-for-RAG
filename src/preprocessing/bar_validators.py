...
class WidthValidator(BarValidator):
    ...
    def validate(self, bars: List[Dict]) -> List[Dict]:
        ...
        filtered_bars = [
            b for b in bars 
            if min_width <= b['bbox'][2] <= max_width
        ]
...
class SpacingValidator(BarValidator):
    ...
    def validate(self, bars: List[Dict]) -> List[Dict]:
        ...
        for i in range(1, len(sorted_bars)):
            spacing_before = sorted_bars[i]['center'][0] - sorted_bars[i-1]['center'][0]
            ...
            if spacing_deviation < 0.5:  # Within 50% of average
                ...
                width_deviation = abs(sorted_bars[i]['bbox'][2] - median_width) / median_width
